...
X = 6     # distance on X axis
Y = 4     # distance on Y axis


delta_x = 0.2     # node to node length in X-axis
delta_y = 0.2     # node to node length in Y-axis
...
# Bottom BC:
x11 = 1     # Till what distance the values are T11
x12 = 1.2     # From what distance the values are T12
T11 = 0     # Temp till x11
T12 = 100     # Temp from x12

i11 = round(x11/delta_x)
i12 = round(x12/delta_x)

# Left BC:
T21 = 0     # Temp on left Boundary
...
for i in range(M):          # Bottom and Top Boundary Conditions
    ...
    if i>=i12:
        DOMAIN[i,0] = T12
    if i>i11 and i<i12:
        DOMAIN[i,0] = (i-i11)/(i12-i11)*(T12-T11)


for j in range(N):          # Left and Right Boundary Conditions
    
    DOMAIN[0,j] = T21       # TOP BC
    
# 4. The right edge boundary needs to be updated in the iteration
...
while True:
    ...
    for j in range(N):
        J_domain[M-1,j] = (1/3)*(4*J_domain[M-2,j] - J_domain[M-3,j])
...
with open("Stream-Jacobian.plt", "w") as f:
    f.write("VARIABLES = \"X\", \"Y\", \"PSI\"\n")
...
while True:
    ...
    # RIGHT Boundary Condition (Back Difference second order df/dx=0)
    for j in range(N):
        PGS_domain[M-1,j] = (1/3)*(4*PGS_domain[M-2,j] - PGS_domain[M-3,j])
...
### Creating the .plt file ###
with open("Stream-Point Gauss-Seidel.plt", "w") as f:
    f.write("VARIABLES = \"X\", \"Y\", \"PSI\"\n")
    f.write("ZONE T = \"BLOCK1\", I = %d, J = %d, F = POINT\n"%(M, N))
...
### Creating the .plt file ###
with open("Stream-PSOR.plt", "w") as f:
    f.write("VARIABLES = \"X\", \"Y\", \"PSI\"\n")
    f.write("ZONE T = \"BLOCK1\", I = %d, J = %d, F = POINT\n"%(M, N))
...
#     c[-1] = 0
    ...
    # RIGHT Boundary Condition (Back Difference second order df/dx=0)
    for j in range(N):
        LGS_domain[M-1,j] = (1/3)*(4*LGS_domain[M-2,j] - LGS_domain[M-3,j])
...
### Creating the .plt file ###
with open("Stream-Line Gauss-Seidel.plt", "w") as f:
    f.write("VARIABLES = \"X\", \"Y\", \"PSI\"\n")
    f.write("ZONE T = \"BLOCK1\", I = %d, J = %d, F = POINT\n"%(M, N))
...
#     c[-1] = 0
    ...
    ### RIGHT Boundary Condition (Back Difference second order df/dx=0)
    for j in range(N):
        ADI_domain[M-1,j] = (1/3)*(4*ADI_domain[M-2,j] - ADI_domain[M-3,j])
...
### Creating the .plt file ###
with open("Stream-ADI.plt", "w") as f:
    f.write("VARIABLES = \"X\", \"Y\", \"PSI\"\n")
    f.write("ZONE T = \"BLOCK1\", I = %d, J = %d, F = POINT\n"%(M, N))
...
plt.savefig("Stream-Weights_Iter.jpg")
...
plt.savefig("Stream-Error_vs_Iter.jpg")
...
TimeToRun.to_csv("Stream-Time_to_run.csv", index=False)
